...
def fetch_poster(movie_id):
    response = requests.get(
        'https://api.themoviedb.org/3/movie/{}?api_key=642f1b3979d8872a08c93c3408b38b8a&language=en-US'.format(
            movie_id))
    data = response.json()
...
movies_dict = pickle.load(open('allmovies_dict.pkl', 'rb'))
...
selected_movie_name = st.selectbox(
    'search for your preferred movie',
    movies['title'].values
)
